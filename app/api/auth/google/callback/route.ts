...
export async function GET(request: NextRequest) {
  ...
  if (!isOAuthConfigured) {
    return NextResponse.redirect(new URL('/auth/login?error=google_not_configured', appUrl))
  }
  ...
  if (!code || !state) {
    return NextResponse.redirect(new URL('/auth/login?error=invalid_google_state', appUrl))
  }
  ...
  try {
    ...
    if (!profile.emailVerified) {
      return NextResponse.redirect(new URL('/auth/login?error=google_email_not_verified', appUrl))
    }
