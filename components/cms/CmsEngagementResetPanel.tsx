...
export function CmsEngagementResetPanel({ onError }: CmsEngagementResetPanelProps) {
  ...
  const handleReset = async () => {
    ...
    if (preview.totalVotes === 0 && preview.likes === 0 && preview.views === 0) {
      onError('Немає активних голосів, лайків або переглядів для скидання')
      return
    }
    ...
    const approved = await confirmAction({
      title: 'Скинути голоси, лайки та перегляди на всіх серверах?',
      description: 'Поточні голоси, лайки й перегляди будуть збережені в історії. Відгуки та рейтинг відгуків не зміняться. Дію не можна скасувати.',
    ...
    <section className="cms-maintenance-page">
      <div className="cms-maintenance-hero">
        <div>
          ...
          <h1>Скидання голосів, лайків та переглядів</h1>
          <p>
            Ручне місячне (або сезонне) обнулення голосів, лайків і переглядів на всіх серверах.
            Дані зберігаються в історії, після скидання статистика накопичується з нуля.
          </p>
        </div>
      </div>
      ...
      <div className="cms-maintenance-layout">
        <div className="cms-maintenance-editor">
          ...
          <div className="cms-reset-stats">
            ...
            <div><span>Перегляди</span><b>{preview?.views ?? 0}</b></div>
            <div><span>Серверів</span><b>{preview?.serversCount ?? 0}</b></div>
          </div>
          <p style={{ color: 'var(--fg-3)', fontSize: 13, lineHeight: 1.5 }}>
            Не скидаються: відгуки, середній рейтинг, онлайн-статистика.
          </p>
          <label>
          ...
          <div className="cms-maintenance-actions">
            <button
              className="danger"
              disabled={resetting || !preview || (preview.totalVotes === 0 && preview.likes === 0 && preview.views === 0)}
              onClick={() => void handleReset()}
              type="button"
            >
              {resetting ? 'Скидання...' : 'Скинути статистику'}
            </button>
              ...
              {history.map((batch) => (
                <article className="cms-reset-history-item" key={batch.id}>
                  ...
                  <span>
                    ...
                    {' '}♥ {batch.likesArchived} лайків ·
                    {' '}👁 {batch.viewsArchived} переглядів · {batch.serversCount} серверів
                  </span>
